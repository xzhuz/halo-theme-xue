...
<@layout title="${post.title!} | ${options.blog_title!}">
  <main class="mx-auto" id="container">
    <header class="bg-gray-900 table fade-down relative w-full opacity-95 z-10 h-1/2" id="postHeader">
        <#if post.thumbnail?? && post.thumbnail!=''>
          <div class="cover-bg bottom-0 left-0 right-0 top-0 opacity-30 absolute">
            <img src="${post.thumbnail!}" class="h-full w-full left-0 object-cover absolute top-0" alt="${post.title!}">
          </div>
        <#elseif settings.card_random_cover_list?? && settings.card_random_cover_list != ''>
          <div class="cover-bg bottom-0 left-0 right-0 top-0 opacity-30 absolute">
            <img src="${theme_base!}/source/images/loading.svg" class="h-full w-full left-0 object-cover absolute top-0 img-random" alt="${post.title}"/>
          </div>
        <#else>
      ...
      <div class="h-96 align-middle table-cell relative w-full index flex justify-center">
        <h2 class="text-white mb-5 mt-4 leading-loose relative w-full text-2xl md:text-4xl text-center">
          ${post.title!}
        </h2>
        ...
        <div class="post-meta text-white">
          <div class="post-meta-wrap absolute h-24 text-right w-full bottom-2.5 right-2.5">
            <img class="rounded-full float-right h-16 md:h-20 h-16 md:w-20 mt-1 md:mt-0 border-2 border-white mx-1 ml-3"
                 srcset="${user.avatar!}, ${user.avatar!} 2x"
                 src="${user.avatar!}" alt=""/>
            <span class="tracking-wide uppercase block text-sm my-1 md:my-2">${post.visits} 次访问</span>
            <time class="tracking-wide uppercase block text-sm my-1 md:my-2"> 发布: ${post.createTime?string("yyyy-MM-dd")}</time>
            <time class="tracking-wide uppercase block text-sm my-1 md:my-2" >更新: ${post.editTime?string("yyyy-MM-dd")}</time>
          </div>
    ...
    <div class="article-content fade-up">
      <div  class="mx-auto px-10 md-content mt-8 max-w-5xl tracking-wider md:leading-relaxed sm:leading-normal heti" id="gallery-content">
        ${post.formatContent!}
      </div>
      <div id="tocFlag"></div>
      ...
      <p class="flex flex-row justify-start flex-wrap">
          <#if post.tags?? && post.tags?size gt 0>
              <#list post.tags as tag>
                <a href="${tag.fullPath!}"class="bg-gray-200 hover:shadow-md hover:text-white hover:bg-red-400 mt-2 mb-2 mr-2 block py-0 px-4 rounded leading-10 h-10 transition text-gray-800 no-underline" style="text-decoration: none !important;">
                  #&nbsp;${tag.name!}
                </a>
              </#list>
          </#if>
