        ...
        <div class="nav previous flex flex-col justify-center min-h-100 py-0 px-4 relative w-full sm:w-6/12 break-word">
            <#if prevPost??>
                <#if prevPost.thumbnail?? && prevPost.thumbnail!=''>
                    <img class="lazyload h-full w-full left-0 top-0 object-cover absolute dark:filter-60" src="${theme_base!}/source/images/loading.svg" data-src="${prevPost.thumbnail}" alt="${prevPost.title!}" />
                <#elseif settings.card_random_cover_list?? && settings.card_random_cover_list != ''>
                    <img class="img-random lazyloaded h-full w-full left-0 top-0 object-cover absolute dark:filter-60" index="${.now?string['ss']?number - 1}" src="${theme_base!}/source/images/loading.svg"
                         alt="${prevPost.title!}"/>
                </#if>
        ...
        <div class="nav next items-end text-right flex flex-col justify-center min-h-100 py-0 px-4 relative w-full sm:w-6/12 break-word">
            <#if nextPost??>
                <#if nextPost.thumbnail?? && nextPost.thumbnail!=''>
                    <img class="lazyload h-full w-full left-0 top-0 object-cover absolute dark:filter-60" src="${theme_base!}/source/images/loading.svg" data-src="${nextPost.thumbnail}" alt="${nextPost.title!}"/>
                <#elseif settings.card_random_cover_list?? && settings.card_random_cover_list != ''>
                    <img class="img-random lazyloaded h-full w-full left-0 top-0 object-cover absolute dark:filter-60" index="${.now?string['ss']?number + 1}" src="${theme_base!}/source/images/loading.svg"
                         alt="${nextPost.title!}"/>
                </#if>
            ...
            <#else>
                <#if settings.card_random_cover_list?? && settings.card_random_cover_list != ''>
                    <img class="img-random lazyloaded h-full w-full left-0 top-0 object-cover absolute dark:filter-60" index="${.now?string['ss']?number + 1}" src="${theme_base!}/source/images/loading.svg" alt=""/>
                </#if>
                <span class="text-white font-medium text-lg pt-8 relative uppercase z-10 dark:text-gray-300">下一篇</span>
                <h4 class="text-white text-lg mt-1 mx-0 mb-0 pb-8 relative z-10 dark:text-gray-300">无</h4>
            </#if>
